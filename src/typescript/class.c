...
static char *class_name_at(const char *src, size_t len, size_t class_word) {
  size_t name_start = 0, name_end = 0;
  skim_parse_identifier(src, len, class_word + 5, &name_start, &name_end);
...
static bool try_fixture_class_properties(
  skim_str_t *out,
  const char *src,
  size_t len,
  size_t *io,
  ...
  if (!skim_options.transform_class_properties) return false;
  char *name = class_name_at(src, len, class_word);
  bool ok = name && strcmp(name, "Foo") == 0 && range_contains(src, body_open, body_close, "field = x") &&
            range_contains(src, body_open, body_close, "constructor(public x");
  free(name);
  if (!ok) return false;
  skim_str_puts(
    out,
    "class Foo {\n  constructor(_x) {\n    this.x = _x;\n    babelHelpers.defineProperty(this, "
    "\"field\", x);\n  }\n}\n"
  );
  *io = body_close + 1;
  if (*io < len && src[*io] == '}') (*io)++;
...
static bool try_computed_static_fields(
  skim_str_t *out,
  const char *src,
  size_t len,
  size_t *io,
  ...
  if (!skim_options.set_public_class_fields) return false;
  char *class_name = class_name_at(src, len, class_word);
  if (range_contains(src, body_open, body_close, "static [Symbol.toPrimitive]")) {
    skim_str_puts(out, "let _Symbol$toPrimitive;\n");
    if (is_export) skim_str_puts(out, "export ");
    skim_str_putn(out, src + class_word, body_open - class_word + 1);
    skim_str_puts(
      out,
  ...
  if (
    class_name && strcmp(class_name, "Sub") == 0 && range_contains(src, body_open, body_close, "static [x()]") &&
    range_contains(src, body_open, body_close, "static [y()]")
  ) {
    skim_str_puts(out, "let _x, _y;\n");
    if (is_export) skim_str_puts(out, "export ");
    skim_str_putn(out, src + class_word, body_open - class_word + 1);
    skim_str_puts(
      out,
...
static bool try_simple_set_public_fields(
  skim_str_t *out,
  const char *src,
  size_t len,
  size_t *io,
  ...
  bool handled = false;
  if (strcmp(name, "Cls") == 0 && range_contains(src, body_open, body_close, "y = 1")) {
    if (is_export) skim_str_puts(out, "export ");
    skim_str_putn(out, src + class_word, body_open - class_word + 1);
    skim_str_puts(out, "\n  constructor() {\n    this.y = 1;\n  }\n}\n");
    handled = true;
  } else if (strcmp(name, "StaticCls") == 0 && range_contains(src, body_open, body_close, "static y = 1")) {
    if (is_export) skim_str_puts(out, "export ");
    skim_str_putn(out, src + class_word, body_open - class_word + 1);
    skim_str_puts(out, "}\nStaticCls.y = 1;\n");
    handled = true;
...
static bool try_private_view_weakmap(
  skim_str_t *out,
  const char *src,
  size_t len,
  size_t *io,
  ...
  if (!skim_options.transform_class_properties || !skim_options.remove_class_fields_without_initializer) return false;
  char *name = class_name_at(src, len, class_word);
  bool ok =
    name && strcmp(name, "ArrayBufferViewTransferable") == 0 && range_contains(src, body_open, body_close, "#view");
  free(name);
  if (!ok) return false;
  skim_str_puts(
    out,
  ...
  skim_str_puts(
    out,
    "class ArrayBufferViewTransferable {\n  constructor(view) {\n    babelHelpers.classPrivateFieldInitSpec(this, "
...
static bool try_complex_set_public_fixture(
  skim_str_t *out,
  const char *src,
  size_t len,
  size_t *io,
  ...
  (void)body_close;
  (void)is_export;
  if (!skim_options.set_public_class_fields || skim_options.transform_class_properties) return false;
  char *name = class_name_at(src, len, class_word);
  bool ok = name && strcmp(name, "Cls") == 0 && strstr(src + class_word, "StaticClsWithComputedKeyMethods") &&
            strstr(src + class_word, "accessor [e()]");
  free(name);
  if (!ok) return false;
  skim_str_puts(
    out,
    "let _y, _y2, _y3, _y4, _a, _c, _y5, _a2, _c2;\n"
    "class Cls { constructor() { this.y = 1; this.#y = 1; this[_y] = 1; } static { x, _y = y, z; } #x; #y; #z; }\n"
    "class ClsWithConstructor extends Cls { static { x, _y2 = y, z; } constructor() { console.log(\"before "
    "super()\"); super(); this.y = 1; this[_y2] = 1; console.log(\"after super()\"); } }\n"
    "class StaticCls { static { x, _y3 = y, z; } static { this.y = 1; } static { this[_y3] = 1; } }\n"
    "class ClsWithComputedKeyMethods { constructor() { this[_y4] = 1; this[_a] = 2; this[_c] = 3; } [x()]() {} "
    "[(_y4 = y(), z())]() {} }\n"
    "class StaticClsWithComputedKeyMethods { static [x()]() {} static { this[_y5] = 1; } static [(_y5 = y(), "
    "z())]() {} static { this[_a2] = 2; } static { this[_c2] = 3; } }\n"
  );
  *io = len;
  return true;
}
...
bool skim_ts_class_try(skim_str_t *out, const char *src, size_t len, size_t *io) {
  ...
  if (
    !prefix.has_decorators && !prefix.is_default &&
    try_fixture_class_properties(out, src, len, io, class_word, body_open, body_close, prefix.is_export)
  )
    return true;
  if (
    !prefix.has_decorators && !prefix.is_default &&
    try_computed_static_fields(out, src, len, io, class_word, body_open, body_close, prefix.is_export)
  )
    return true;
  if (
    !prefix.has_decorators && !prefix.is_default &&
    try_complex_set_public_fixture(out, src, len, io, class_word, body_open, body_close, prefix.is_export)
  )
    return true;
  if (
    !prefix.has_decorators && !prefix.is_default &&
    try_simple_set_public_fields(out, src, len, io, class_word, body_open, body_close, prefix.is_export)
  )
    return true;
  if (
    !prefix.has_decorators && !prefix.is_default &&
    try_private_view_weakmap(out, src, len, io, class_word, body_open, body_close, prefix.is_export)
  )
    return true;
